...
export async function POST(request: NextRequest, { params }: Params) {
  ...
  // Prevent self-voting for MVP (DB also enforces this via CHECK constraint)
  if (vote_type === 'mvp' && voted_for_profile_id === user.id) {
    return NextResponse.json({ data: null, error: { code: 'BAD_REQUEST', message: 'Cannot vote for yourself for MVP' } }, { status: 400 });
  }
